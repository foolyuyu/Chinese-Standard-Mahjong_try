...
def transform_sample(sample, variant):
    ...
    flat_obs = obs.reshape(OBS_SHAPE[0], OBS_TILE_COUNT)
    transformed_obs = flat_obs[:, variant['tile_new_to_old']].reshape(OBS_SHAPE)
...
def transform_batch(batch, variant):
    obs = batch['obs']
    flat_obs = obs.reshape(obs.shape[0], OBS_SHAPE[0], OBS_TILE_COUNT)
    transformed_obs = flat_obs[:, :, variant['tile_new_to_old']].reshape(obs.shape)
    ...
    return {
        'obs': transformed_obs.astype(obs.dtype, copy = False),
